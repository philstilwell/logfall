...
def build_prompt(item: dict) -> str:
    ...
    return (
        ...
        "Two unique humorous cartoon speakers must be fully visible from head to shoes: one on the far left edge and one on the far right edge. "
        "Leave the entire center area open and clean for six future dialogue boxes arranged in a left-right-left-right-left-right staircase. "
        "Do not draw any speech bubbles, text, letters, numbers, captions, labels, signs, or tail shapes. "
        "No cropping of either speaker. No background clutter. Only subtle ground shadows if needed. "
        ...
        "The middle must remain clean and spacious enough for six stacked rounded dialogue rectangles."
    )
    ...
    with OUT_PATH.open("w", encoding="utf-8") as fh:
        for item in bank:
            job = {
                ...
                "composition": "wide landscape scene, fully visible left and right speakers, broad empty central space, no speech bubbles or text",
                "constraints": "pure white background; grayscale only; no printed text; no captions; no tails; no cropping; leave center open for six future dialogue boxes",
                "size": "1536x1024",
                "quality": "high",
                "output_format": "png",
